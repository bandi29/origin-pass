...
import { isValidUuid, sanitizeFilename, sanitizeForFilename } from "@/lib/security"
...
export async function GET(
  ...
  const manifest = buildManifest(batchData, exportDate)
  zip.file("manifest.json", JSON.stringify(manifest, null, 2))

  // Add one JSON-LD file per serial number (sanitized filename prevents path traversal)
  for (const item of itemList) {
    const jsonLd = mapItemToJsonLd(item, batchData, baseUrl)
    zip.file(`${sanitizeForFilename(item.serial_id)}.json`, JSON.stringify(jsonLd, null, 2))
  }

  const zipBuffer = await zip.generateAsync({ type: "uint8array" })
  const safeName = sanitizeFilename(batch.production_run_name ?? batchId.slice(0, 8))
  const filename = `originpass-dpp-export-${safeName}.zip`
